...
class RLAgent(object):
    ...
    def __init__(self,
        ...
        Inputs:
            ...
            prt: 控制输出并且将日志写入文件
            env: 一个环境的实例
            dataGen: 一个数据生成器，用来生成数据来测试或者训练
            reward_func: 一个被用来计算奖励的函数。在VRP任务中返回旅行长度
            clAttentionActor:在actor中的注意力机制
            clAttentionCritic: 在批评网络中的注意力机制
            is_train: 如果是ture，用来训练，否则值用来推断
        '''
        ...
        self.prt.print_out("花费 {}秒 建立代理.".format(str(model_time)))
        ...
        for i in range(args['decode_len']):
            ...
            if decode_type == 'greedy':
                ...
                def my_multinomial():
                    ...
                    prob_idx_cum = tf.cumsum(prob_idx, 1)  # 沿着axis=1计算累积和,1,2,3 累积和就是 1,3,6
                ...
                # 检查 tmp 的有效性 -> True or False -- True mean take a new sample
    ...
    def Initialize(self, sess):
        self.sess = sess
        self.sess.run(tf.global_variables_initializer())  # tf.global_variables_initializer是初始化变量
        self.load_model()

    def load_model(self):
        latest_ckpt = tf.train.latest_checkpoint(
            self.args['load_path'])  # tf.train.latest_checkpoint自动找到最近保存的变量文件，load_path是目录
        if latest_ckpt is not None:  # 如果存在
            self.saver.restore(self.sess, latest_ckpt)  # 取出模型参数
    ...
    def evaluate_single(self, eval_type='greedy'):
        ...
        for step in range(self.dataGen.n_problems):
            ...
            if step % int(self.args['log_interval']) == 0:
                ...
                self.prt.print_out('\n\n搜索方式-步数 of {}: {}'.format(eval_type, step))
                self.prt.print_out('\n测试输入示例: {}'.format(example_input))
                self.prt.print_out('\n测试输出示例: {}'.format(example_output))
                self.prt.print_out('\n测试奖励示例: {} - best: {}'.format(R[0], R_ind0))

        end_time = time.time() - start_time

        # 完成了对迭代器数据集的遍历。
        self.prt.print_out('\n验证总体平均奖励: {}'.format(np.mean(avg_reward)))
        self.prt.print_out('验证总体奖励标准: {}'.format(np.sqrt(np.var(avg_reward))))

        self.prt.print_out("已完成评估，用 %d 步 , 用时 %s ." % (step \
    ...
    def evaluate_batch(self, eval_type='greedy'):
        ...
        R = np.amin(R, 1, keepdims=False)  # 获得行的最小值

        end_time = time.time() - start_time
        self.prt.print_out('在批处理模式中 {} 的平均值: {} -- std {} -- time {} s'.format(eval_type, \
